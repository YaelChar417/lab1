...
-- query para ver que proveedores entregan un cierto material especifico.
SELECT p.rfc, p.razonsocial, m.descripcion, (e.cantidad * (m.precio * m.impuesto)) AS total, e.fecha
FROM proveedores AS p, materiales AS m, entregan AS e
WHERE m.clave = e.clave
AND p.rfc = e.rfc 
AND m.clave = 1000;

-- query para ver que materiales se han entregado a un proyecto en especifico.
SELECT m.clave, m.descripcion, p.razonsocial, e.fecha
FROM materiales AS m, proveedores AS p, entregan AS e
WHERE m.clave = e.clave
AND e.rfc = p.rfc
AND p.rfc = 'EEEE800101';

-- query para ver cuando entrego un provedor en especifico a los proyectos.
SELECT prov.rfc, prov.razonsocial, proy.denominacion, e.fecha
FROM proveedores AS prov, proyectos as proy, entregan AS e
WHERE prov.rfc = e.rfc
AND e.numero = proy.numero
AND prov.rfc = 'FFFF800101';